...
void vme_open()
{
  ...
  //   if(status!=GEF_STATUS_SUCCESS){
  //     sprintf(message, "vme05: V792: gefVmeCreateMasterWindow() failed -- %d", status);
  //     send_fatal_message(message);
  //     std::exit(-1);
  //   }
  //   status = gefVmeMapMasterWindow(mst_hdl[hdl_num], 0, w_size, &map_hdl[hdl_num], &ptr);
  //   if(status!=GEF_STATUS_SUCCESS){
  //     sprintf(message, "vme05: V792: gefVmeMapMasterWindow() failed -- %d", GEF_GET_ERROR(status));
  //     send_fatal_message(message);
  //     std::exit(-1);
  //   }
  //   hdl_num++;
  ...
  //     sprintf(message, "vme05: V792    [%08llx] joined", v792[i].addr);
  //     send_normal_message(message);
  //   }
  // }
  //### TDC64M #################################################
