...
RPV130_REG rpv130[RPV130_NUM] = { { 0x0000E030 } };
V792_REG   v792[V792_NUM]     = { { 0xAD050000 }, { 0xAD060000 },
				  { 0xAD070000 }, { 0xAD080000 } };
TDC64M_REG tdc64m[TDC64M_NUM] = { { 0x43000000 }, { 0x44000000 } };
...
void vme_open()
{
  ...
  if(status!=GEF_STATUS_SUCCESS){
    sprintf(pbuf, "vme05: gefVmeOpen() failed -- %d", GEF_GET_ERROR(status));
    send_fatal(pbuf);
    std::exit(-1);
  }
  status = gefVmeAllocDmaBuf(bus_hdl, 4*DMA_BUF_LEN, &dma_hdl, (GEF_MAP_PTR*)&dma_buf);
  if(status!=GEF_STATUS_SUCCESS){
    sprintf(pbuf, "vme05: gefVmeAllocDmaBuf() failed -- %d", GEF_GET_ERROR(status));
    send_fatal(pbuf);
    std::exit(-1);
  }
  //### VME_RM #################################################
  {
    ...
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: VME_RM: gefVmeCreateMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
    status = gefVmeMapMasterWindow(mst_hdl[hdl_num], 0, w_size, &map_hdl[hdl_num], &ptr);
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: VME_RM: gefVmeMapMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
    hdl_num++;
    for(int i=0;i<VME_RM_NUM;i++){
      ...
      sprintf(pbuf, "vme05: VME_RM  [%08llx] joined", vme_rm[i].addr);
      send_normal(pbuf);
    }
  }
  //### RPV130 #################################################
  {
    ...
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: RPV130: gefVmeCreateMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
    status = gefVmeMapMasterWindow(mst_hdl[hdl_num], 0, w_size, &map_hdl[hdl_num], &ptr);
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: RPV130: gefVmeMapMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
    hdl_num++;
    for(int i=0;i<RPV130_NUM;i++){
      ...
      sprintf(pbuf, "vme05: RPV130  [%08llx] joined", rpv130[i].addr);
      send_normal(pbuf);
    }
  }
  //### V792 #################################################
  {
    ...
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: V792: gefVmeCreateMasterWindow() failed -- %d", status);
      send_fatal(pbuf);
      std::exit(-1);
    }
    status = gefVmeMapMasterWindow(mst_hdl[hdl_num], 0, w_size, &map_hdl[hdl_num], &ptr);
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: V792: gefVmeMapMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
    hdl_num++;
    for(int i=0;i<V792_NUM;i++){
      ...
      sprintf(pbuf, "vme05: V792    [%08llx] joined", v792[i].addr);
      send_normal(pbuf);
    }
  }
  //### TDC64M #################################################
  {
    ...
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: TDC64M: gefVmeCreateMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
    status = gefVmeMapMasterWindow(mst_hdl[hdl_num], 0, w_size, &map_hdl[hdl_num], &ptr);
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: TDC64M: gefVmeMapMasterWindow() failed, status: %d \n", status);
      send_fatal(pbuf);
      std::exit(-1);
    }
    hdl_num++;
    for(int i=0;i<TDC64M_NUM;i++){
      ...
      sprintf(pbuf, "vme05: TDC64M  [%08llx] joined", tdc64m[i].addr);
      send_normal(pbuf);
    }
  }
}
...
void vme_close()
{
  ...
  if(status!=GEF_STATUS_SUCCESS){
    sprintf(pbuf, "vme05: gefVmeFreeDmaBuf() failed -- %d \n", GEF_GET_ERROR(status));
    send_fatal(pbuf);
    std::exit(-1);
  }
  for(int i=0;i<hdl_num;i++){
    ...
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: gefVmeUnmapMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
    status = gefVmeReleaseMasterWindow( mst_hdl[i] );
    if(status!=GEF_STATUS_SUCCESS){
      sprintf(pbuf, "vme05: gefVmeReleaseMasterWindow() failed -- %d", GEF_GET_ERROR(status));
      send_fatal(pbuf);
      std::exit(-1);
    }
  }
  ...
  if(status!=GEF_STATUS_SUCCESS){
    sprintf(pbuf, "vme05: gefVmeClose() failed -- %d", GEF_GET_ERROR(status));
    send_fatal(pbuf);
    std::exit(-1);
  }
}
...
void check_handle_number(int handle_number)
{
  if(handle_number>=max_hdl_num){
    char pbuf[256];
    sprintf(pbuf, "vme05: too many Master Windows -- %d/%d",
