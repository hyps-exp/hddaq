  ...
  case DM_NORMAL:
	...
	if( (reg>>0)&0x1 ){
	  *(rpv130[0].csr1)  = __bswap_16(0x01); // clear
      ...
      // TimeOut
      send_warning("vme05: wait_device() Time Out");
      return -1;
    }
  case DM_DUMMY:
    {
  ...
  case DM_NORMAL:
	...
	for(int i=0;i<V792_NUM;i++){
	  ...
	  if(dready==1){
	    ...
	    if(status!=0){
	      sprintf(pbuf, "vme05: V792[%08llx] gefVmeReadDmaBuf() failed -- %d",
	    ...
	    sprintf(pbuf, "vme05: V792[%08llx] data is not ready", v792[i].addr );
	    send_warning(pbuf);
	  }
#else
	  for(int j=0;j<data_len;j++){
	...
	for(int i=0;i<TDC64M_NUM;i++){
	  ...
	  if(dready==1){
	    ...
	    if(data_len>DMA_BUF_LEN){
	      sprintf(pbuf, "vme05: TDC64M[%08llx] data_len is too much -- %d/%d",
	    ...
	    if(status!=0){
	      sprintf(pbuf, "vme05: TDC64M[%08llx] gefVmeReadDmaBuf() failed -- %d",
	    ...
	    sprintf(pbuf, "vme05: TDC64M[%08llx] data is not ready", tdc64m[i].addr );
	    send_warning(pbuf);
	  }
