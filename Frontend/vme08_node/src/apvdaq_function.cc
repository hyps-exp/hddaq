...
void read_2fifo_pio(unsigned long module)
{
  ...
  for(i=0;i<50;i++){
    ...
    //adc_A = dataword&0x3ff;
    //adc_B = (dataword>>16)&0x3ff;
	...
	for( chip = 0;chip < APVCHIPNUMB; chip++ )
	  {
	    ...
	    unsigned int dev_adc_threshold;
	    dev_adc_threshold = (unsigned int)(DevMean[module][chip][channel]+threshold*DevRms[module][chip][channel]); 
