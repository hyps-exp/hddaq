...
// 			ReaderThread **readers = m_readers;
		...
		if (checkEventNumber()) {
			//std::cerr << "#E Event Number Missmatch !!" << std::endl;
			char emessage[128];
			sprintf(emessage, "#E BT: Event Number Missmatch !!");
			msock.sendString(MT_ERROR, emessage);
			fprintf(stderr, "%s\n", emessage);
		}
		...
		for (int node = 0; node < m_node_num; node++) {
			...
			memcpy(ptr, reinterpret_cast< char *>
